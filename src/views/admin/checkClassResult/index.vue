  ...
  <div class="app-container">
    ...
    <div class="result">
      ...
      <el-row style="margin: 20px 0">
        <el-button type="primary"  @click="editClass">手动审核</el-button>
      </el-row>
      <MyTable ref="table" :data="classDealList" :config="classDealTableConfig" :page="false" >
        <template slot="operate">
          <el-table-column label="操作" width="280" align="center">
            <template slot-scope="scope">
              <el-link style="margin-left: 10px" @click="confirmLabel(scope.row.id)" type="primary" >确认</el-link>
              <el-link style="margin-left: 10px" @click="check(scope.row.id)" type="primary">查看</el-link>
            </template>
          </el-table-column>
        </template>
      </MyTable>
...
export default {
  ...
  methods: {
    ...
    getClassResult(id) {
      request({
        url: '/api/classText/getCheckLabel/' + id,
        method: 'get'
      }).then(res => {
        const { code } = res
        if (code === 200) {
    ...
    confirmLabel(id) {
      request({
        url: '/api/auditText/confirmCheckLabel/' + this.text_id + '/' + id,
        method: 'get'
      }).then(res => {
        const { code } = res
        if (code === 200) {
    ...
    editClassResult() {
      ...
      request({
        url: '/api/auditText/confirmCheckLabel/' + this.text_id + '/' + classID,
        method: 'get'
      }).then(res => {
        const { code } = res
